// Combo of my own research into undocumented NT Apis and SysWhispers header
// https://github.com/jthuraisamy/SysWhispers/blob/master/example-output/syscalls.h
// 
// Defines D_API, NT_API, and SYSCALL_TABLE as well
//	- D_API allows convenient function pointer function member types
//  - NT_API is a struct of pointers to all known NT_APIs at the time of 
//		this file's creation
//	- SYSCALL_TABLE is just a structure for storing all syscall names and numbers
//
#pragma once
#include <Windows.h>
#include <type_traits>

#define D_API(x) std::remove_pointer<decltype(&x)>::type
...
EXTERN_C NTSTATUS NtQueueApcThreadEx2(
...
EXTERN_C NTSTATUS NtAcquireCrossVmMutant(
	IN HANDLE MutantHandle,
	IN PLARGE_INTEGER Timeout OPTIONAL
);
...
EXTERN_C NTSTATUS NtAllocateUserPhysicalPagesEx(
	IN HANDLE ProcessHandle,
	IN PULONG_PTR NumberOfPages,
	IN PULONG_PTR PageFrameNumbers
);

EXTERN_C NTSTATUS NtCopyFileChunk(
	IN HANDLE SourceFileHandle,
	IN HANDLE DestFileHandle,
	IN PLARGE_INTEGER SourceOffset,
	IN PLARGE_INTEGER DestinationOffset,
	IN PLARGE_INTEGER CopyLength,
	IN ULONG Flags
);

typedef PHANDLE GRAPHICS_DEVICE_HANDLE;

EXTERN_C NTSTATUS NtDirectGraphicsCall(
	IN GRAPHICS_DEVICE_HANDLE DeviceHandle,
	IN PVOID InputBuffer,
...
EXTERN_C NTSTATUS NtCreateCrossVmMutant(
	OUT PHANDLE MutantHandle,
	IN ACCESS_MASK DesiredAccess,
	IN POBJECT_ATTRIBUTES ObjectAttributes,
	IN ULONG Flags
...
EXTERN_C NTSTATUS NtPssCaptureVaSpaceBulk(HANDLE ProcessHandle,
	PVOID BaseAddress,
...
{
	D_API(NtAcceptConnectPort)* NtAcceptConnectPort;
	D_API(NtAccessCheck)* NtAccessCheck;
	D_API(NtAccessCheckAndAuditAlarm)* NtAccessCheckAndAuditAlarm;
	D_API(NtAccessCheckByType)* NtAccessCheckByType;
	D_API(NtAccessCheckByTypeAndAuditAlarm)* NtAccessCheckByTypeAndAuditAlarm;
	D_API(NtAccessCheckByTypeResultList)* NtAccessCheckByTypeResultList;
	D_API(NtAccessCheckByTypeResultListAndAuditAlarm)* NtAccessCheckByTypeResultListAndAuditAlarm;
	D_API(NtAccessCheckByTypeResultListAndAuditAlarmByHandle)* NtAccessCheckByTypeResultListAndAuditAlarmByHandle;
	D_API(NtAcquireCrossVmMutant)* NtAcquireCrossVmMutant;
	D_API(NtAcquireProcessActivityReference)* NtAcquireProcessActivityReference;
	D_API(NtAddAtom)* NtAddAtom;
	D_API(NtAddAtomEx)* NtAddAtomEx;
	D_API(NtAddBootEntry)* NtAddBootEntry;
	D_API(NtAddDriverEntry)* NtAddDriverEntry;
	D_API(NtAdjustGroupsToken)* NtAdjustGroupsToken;
	D_API(NtAdjustPrivilegesToken)* NtAdjustPrivilegesToken;
	D_API(NtAdjustTokenClaimsAndDeviceGroups)* NtAdjustTokenClaimsAndDeviceGroups;
	D_API(NtAlertResumeThread)* NtAlertResumeThread;
	D_API(NtAlertThread)* NtAlertThread;
	D_API(NtAlertThreadByThreadId)* NtAlertThreadByThreadId;
	D_API(NtAllocateLocallyUniqueId)* NtAllocateLocallyUniqueId;
	D_API(NtAllocateReserveObject)* NtAllocateReserveObject;
	D_API(NtAllocateUserPhysicalPages)* NtAllocateUserPhysicalPages;
	D_API(NtAllocateUserPhysicalPagesEx)* NtAllocateUserPhysicalPagesEx;
	D_API(NtAllocateUuids)* NtAllocateUuids;
	D_API(NtAllocateVirtualMemory)* NtAllocateVirtualMemory;
	D_API(NtAllocateVirtualMemoryEx)* NtAllocateVirtualMemoryEx;
	D_API(NtAlpcAcceptConnectPort)* NtAlpcAcceptConnectPort;
	D_API(NtAlpcCancelMessage)* NtAlpcCancelMessage;
	D_API(NtAlpcConnectPort)* NtAlpcConnectPort;
	D_API(NtAlpcConnectPortEx)* NtAlpcConnectPortEx;
	D_API(NtAlpcCreatePort)* NtAlpcCreatePort;
	D_API(NtAlpcCreatePortSection)* NtAlpcCreatePortSection;
	D_API(NtAlpcCreateResourceReserve)* NtAlpcCreateResourceReserve;
	D_API(NtAlpcCreateSectionView)* NtAlpcCreateSectionView;
	D_API(NtAlpcCreateSecurityContext)* NtAlpcCreateSecurityContext;
	D_API(NtAlpcDeletePortSection)* NtAlpcDeletePortSection;
	D_API(NtAlpcDeleteResourceReserve)* NtAlpcDeleteResourceReserve;
	D_API(NtAlpcDeleteSectionView)* NtAlpcDeleteSectionView;
	D_API(NtAlpcDeleteSecurityContext)* NtAlpcDeleteSecurityContext;
	D_API(NtAlpcDisconnectPort)* NtAlpcDisconnectPort;
	D_API(NtAlpcImpersonateClientContainerOfPort)* NtAlpcImpersonateClientContainerOfPort;
	D_API(NtAlpcImpersonateClientOfPort)* NtAlpcImpersonateClientOfPort;
	D_API(NtAlpcOpenSenderProcess)* NtAlpcOpenSenderProcess;
	D_API(NtAlpcOpenSenderThread)* NtAlpcOpenSenderThread;
	D_API(NtAlpcQueryInformation)* NtAlpcQueryInformation;
	D_API(NtAlpcQueryInformationMessage)* NtAlpcQueryInformationMessage;
	D_API(NtAlpcRevokeSecurityContext)* NtAlpcRevokeSecurityContext;
	D_API(NtAlpcSendWaitReceivePort)* NtAlpcSendWaitReceivePort;
	D_API(NtAlpcSetInformation)* NtAlpcSetInformation;
	D_API(NtApphelpCacheControl)* NtApphelpCacheControl;
	D_API(NtAreMappedFilesTheSame)* NtAreMappedFilesTheSame;
	D_API(NtAssignProcessToJobObject)* NtAssignProcessToJobObject;
	D_API(NtAssociateWaitCompletionPacket)* NtAssociateWaitCompletionPacket;
	D_API(NtCallEnclave)* NtCallEnclave;
	D_API(NtCallbackReturn)* NtCallbackReturn;
	D_API(NtCancelIoFile)* NtCancelIoFile;
	D_API(NtCancelIoFileEx)* NtCancelIoFileEx;
	D_API(NtCancelSynchronousIoFile)* NtCancelSynchronousIoFile;
	D_API(NtCancelTimer)* NtCancelTimer;
	D_API(NtCancelTimer2)* NtCancelTimer2;
	D_API(NtCancelWaitCompletionPacket)* NtCancelWaitCompletionPacket;
	D_API(NtClearEvent)* NtClearEvent;
	D_API(NtClose)* NtClose;
	D_API(NtCloseObjectAuditAlarm)* NtCloseObjectAuditAlarm;
	D_API(NtCommitComplete)* NtCommitComplete;
	D_API(NtCommitEnlistment)* NtCommitEnlistment;
	D_API(NtCommitRegistryTransaction)* NtCommitRegistryTransaction;
	D_API(NtCommitTransaction)* NtCommitTransaction;
	D_API(NtCompactKeys)* NtCompactKeys;
	D_API(NtCompareObjects)* NtCompareObjects;
	D_API(NtCompareSigningLevels)* NtCompareSigningLevels;
	D_API(NtCompareTokens)* NtCompareTokens;
	D_API(NtCompleteConnectPort)* NtCompleteConnectPort;
	D_API(NtCompressKey)* NtCompressKey;
	D_API(NtConnectPort)* NtConnectPort;
	D_API(NtContinue)* NtContinue;
	D_API(NtContinueEx)* NtContinueEx;
	D_API(NtConvertBetweenAuxiliaryCounterAndPerformanceCounter)* NtConvertBetweenAuxiliaryCounterAndPerformanceCounter;
	D_API(NtCopyFileChunk)* NtCopyFileChunk;
	D_API(NtCreateCrossVmEvent)* NtCreateCrossVmEvent;
	D_API(NtCreateCrossVmMutant)* NtCreateCrossVmMutant;
	D_API(NtCreateDebugObject)* NtCreateDebugObject;
	D_API(NtCreateDirectoryObject)* NtCreateDirectoryObject;
	D_API(NtCreateDirectoryObjectEx)* NtCreateDirectoryObjectEx;
	D_API(NtCreateEnclave)* NtCreateEnclave;
	D_API(NtCreateEnlistment)* NtCreateEnlistment;
	D_API(NtCreateEvent)* NtCreateEvent;
	D_API(NtCreateEventPair)* NtCreateEventPair;
	D_API(NtCreateFile)* NtCreateFile;
	D_API(NtCreateIRTimer)* NtCreateIRTimer;
	D_API(NtCreateIoCompletion)* NtCreateIoCompletion;
	D_API(NtCreateJobObject)* NtCreateJobObject;
	D_API(NtCreateJobSet)* NtCreateJobSet;
	D_API(NtCreateKey)* NtCreateKey;
	D_API(NtCreateKeyTransacted)* NtCreateKeyTransacted;
	D_API(NtCreateKeyedEvent)* NtCreateKeyedEvent;
	D_API(NtCreateLowBoxToken)* NtCreateLowBoxToken;
	D_API(NtCreateMailslotFile)* NtCreateMailslotFile;
	D_API(NtCreateMutant)* NtCreateMutant;
	D_API(NtCreateNamedPipeFile)* NtCreateNamedPipeFile;
	D_API(NtCreatePagingFile)* NtCreatePagingFile;
	D_API(NtCreatePartition)* NtCreatePartition;
	D_API(NtCreatePort)* NtCreatePort;
	D_API(NtCreatePrivateNamespace)* NtCreatePrivateNamespace;
	D_API(NtCreateProcess)* NtCreateProcess;
	D_API(NtCreateProcessEx)* NtCreateProcessEx;
	D_API(NtCreateProfile)* NtCreateProfile;
	D_API(NtCreateProfileEx)* NtCreateProfileEx;
	D_API(NtCreateRegistryTransaction)* NtCreateRegistryTransaction;
	D_API(NtCreateResourceManager)* NtCreateResourceManager;
	D_API(NtCreateSection)* NtCreateSection;
	D_API(NtCreateSectionEx)* NtCreateSectionEx;
	D_API(NtCreateSemaphore)* NtCreateSemaphore;
	D_API(NtCreateSymbolicLinkObject)* NtCreateSymbolicLinkObject;
	D_API(NtCreateThread)* NtCreateThread;
	D_API(NtCreateThreadEx)* NtCreateThreadEx;
	D_API(NtCreateTimer)* NtCreateTimer;
	D_API(NtCreateTimer2)* NtCreateTimer2;
	D_API(NtCreateToken)* NtCreateToken;
	D_API(NtCreateTokenEx)* NtCreateTokenEx;
	D_API(NtCreateTransaction)* NtCreateTransaction;
	D_API(NtCreateTransactionManager)* NtCreateTransactionManager;
	D_API(NtCreateUserProcess)* NtCreateUserProcess;
	D_API(NtCreateWaitCompletionPacket)* NtCreateWaitCompletionPacket;
	D_API(NtCreateWaitablePort)* NtCreateWaitablePort;
	D_API(NtCreateWnfStateName)* NtCreateWnfStateName;
	D_API(NtCreateWorkerFactory)* NtCreateWorkerFactory;
	D_API(NtDebugActiveProcess)* NtDebugActiveProcess;
	D_API(NtDebugContinue)* NtDebugContinue;
	D_API(NtDelayExecution)* NtDelayExecution;
	D_API(NtDeleteAtom)* NtDeleteAtom;
	D_API(NtDeleteBootEntry)* NtDeleteBootEntry;
	D_API(NtDeleteDriverEntry)* NtDeleteDriverEntry;
	D_API(NtDeleteFile)* NtDeleteFile;
	D_API(NtDeleteKey)* NtDeleteKey;
	D_API(NtDeleteObjectAuditAlarm)* NtDeleteObjectAuditAlarm;
	D_API(NtDeletePrivateNamespace)* NtDeletePrivateNamespace;
	D_API(NtDeleteValueKey)* NtDeleteValueKey;
	D_API(NtDeleteWnfStateData)* NtDeleteWnfStateData;
	D_API(NtDeleteWnfStateName)* NtDeleteWnfStateName;
	D_API(NtDeviceIoControlFile)* NtDeviceIoControlFile;
	D_API(NtDirectGraphicsCall)* NtDirectGraphicsCall;
	D_API(NtDisableLastKnownGood)* NtDisableLastKnownGood;
	D_API(NtDisplayString)* NtDisplayString;
	D_API(NtDrawText)* NtDrawText;
	D_API(NtDuplicateObject)* NtDuplicateObject;
	D_API(NtDuplicateToken)* NtDuplicateToken;
	D_API(NtEnableLastKnownGood)* NtEnableLastKnownGood;
	D_API(NtEnumerateBootEntries)* NtEnumerateBootEntries;
	D_API(NtEnumerateDriverEntries)* NtEnumerateDriverEntries;
	D_API(NtEnumerateKey)* NtEnumerateKey;
	D_API(NtEnumerateSystemEnvironmentValuesEx)* NtEnumerateSystemEnvironmentValuesEx;
	D_API(NtEnumerateTransactionObject)* NtEnumerateTransactionObject;
	D_API(NtEnumerateValueKey)* NtEnumerateValueKey;
	D_API(NtExtendSection)* NtExtendSection;
	D_API(NtFilterBootOption)* NtFilterBootOption;
	D_API(NtFilterToken)* NtFilterToken;
	D_API(NtFilterTokenEx)* NtFilterTokenEx;
	D_API(NtFindAtom)* NtFindAtom;
	D_API(NtFlushBuffersFile)* NtFlushBuffersFile;
	D_API(NtFlushBuffersFileEx)* NtFlushBuffersFileEx;
	D_API(NtFlushInstallUILanguage)* NtFlushInstallUILanguage;
	D_API(NtFlushInstructionCache)* NtFlushInstructionCache;
	D_API(NtFlushKey)* NtFlushKey;
	D_API(NtFlushProcessWriteBuffers)* NtFlushProcessWriteBuffers;
	D_API(NtFlushVirtualMemory)* NtFlushVirtualMemory;
	D_API(NtFlushWriteBuffer)* NtFlushWriteBuffer;
	D_API(NtFreeUserPhysicalPages)* NtFreeUserPhysicalPages;
	D_API(NtFreeVirtualMemory)* NtFreeVirtualMemory;
	D_API(NtFreezeRegistry)* NtFreezeRegistry;
	D_API(NtFreezeTransactions)* NtFreezeTransactions;
	D_API(NtFsControlFile)* NtFsControlFile;
	D_API(NtGetCachedSigningLevel)* NtGetCachedSigningLevel;
	D_API(NtGetCompleteWnfStateSubscription)* NtGetCompleteWnfStateSubscription;
	D_API(NtGetContextThread)* NtGetContextThread;
	D_API(NtGetCurrentProcessorNumber)* NtGetCurrentProcessorNumber;
	D_API(NtGetCurrentProcessorNumberEx)* NtGetCurrentProcessorNumberEx;
	D_API(NtGetDevicePowerState)* NtGetDevicePowerState;
	D_API(NtGetMUIRegistryInfo)* NtGetMUIRegistryInfo;
	D_API(NtGetNextProcess)* NtGetNextProcess;
	D_API(NtGetNextThread)* NtGetNextThread;
	D_API(NtGetNlsSectionPtr)* NtGetNlsSectionPtr;
	D_API(NtGetNotificationResourceManager)* NtGetNotificationResourceManager;
	D_API(NtGetWriteWatch)* NtGetWriteWatch;
	D_API(NtImpersonateAnonymousToken)* NtImpersonateAnonymousToken;
	D_API(NtImpersonateClientOfPort)* NtImpersonateClientOfPort;
	D_API(NtImpersonateThread)* NtImpersonateThread;
	D_API(NtInitializeEnclave)* NtInitializeEnclave;
	D_API(NtInitializeNlsFiles)* NtInitializeNlsFiles;
	D_API(NtInitializeRegistry)* NtInitializeRegistry;
	D_API(NtInitiatePowerAction)* NtInitiatePowerAction;
	D_API(NtIsProcessInJob)* NtIsProcessInJob;
	D_API(NtIsSystemResumeAutomatic)* NtIsSystemResumeAutomatic;
	D_API(NtIsUILanguageComitted)* NtIsUILanguageComitted;
	D_API(NtListenPort)* NtListenPort;
	D_API(NtLoadDriver)* NtLoadDriver;
	D_API(NtLoadEnclaveData)* NtLoadEnclaveData;
	D_API(NtLoadKey)* NtLoadKey;
	D_API(NtLoadKey2)* NtLoadKey2;
	D_API(NtLoadKey3)* NtLoadKey3;
	D_API(NtLoadKeyEx)* NtLoadKeyEx;
	D_API(NtLockFile)* NtLockFile;
	D_API(NtLockProductActivationKeys)* NtLockProductActivationKeys;
	D_API(NtLockRegistryKey)* NtLockRegistryKey;
	D_API(NtLockVirtualMemory)* NtLockVirtualMemory;
	D_API(NtMakePermanentObject)* NtMakePermanentObject;
	D_API(NtMakeTemporaryObject)* NtMakeTemporaryObject;
	D_API(NtManageHotPatch)* NtManageHotPatch;
	D_API(NtManagePartition)* NtManagePartition;
	D_API(NtMapCMFModule)* NtMapCMFModule;
	D_API(NtMapUserPhysicalPages)* NtMapUserPhysicalPages;
	D_API(NtMapUserPhysicalPagesScatter)* NtMapUserPhysicalPagesScatter;
	D_API(NtMapViewOfSection)* NtMapViewOfSection;
	D_API(NtMapViewOfSectionEx)* NtMapViewOfSectionEx;
	D_API(NtModifyBootEntry)* NtModifyBootEntry;
	D_API(NtModifyDriverEntry)* NtModifyDriverEntry;
	D_API(NtNotifyChangeDirectoryFile)* NtNotifyChangeDirectoryFile;
	D_API(NtNotifyChangeDirectoryFileEx)* NtNotifyChangeDirectoryFileEx;
	D_API(NtNotifyChangeKey)* NtNotifyChangeKey;
	D_API(NtNotifyChangeMultipleKeys)* NtNotifyChangeMultipleKeys;
	D_API(NtNotifyChangeSession)* NtNotifyChangeSession;
	D_API(NtOpenDirectoryObject)* NtOpenDirectoryObject;
	D_API(NtOpenEnlistment)* NtOpenEnlistment;
	D_API(NtOpenEvent)* NtOpenEvent;
	D_API(NtOpenEventPair)* NtOpenEventPair;
	D_API(NtOpenFile)* NtOpenFile;
	D_API(NtOpenIoCompletion)* NtOpenIoCompletion;
	D_API(NtOpenJobObject)* NtOpenJobObject;
	D_API(NtOpenKey)* NtOpenKey;
	D_API(NtOpenKeyEx)* NtOpenKeyEx;
	D_API(NtOpenKeyTransacted)* NtOpenKeyTransacted;
	D_API(NtOpenKeyTransactedEx)* NtOpenKeyTransactedEx;
	D_API(NtOpenKeyedEvent)* NtOpenKeyedEvent;
	D_API(NtOpenMutant)* NtOpenMutant;
	D_API(NtOpenObjectAuditAlarm)* NtOpenObjectAuditAlarm;
	D_API(NtOpenPartition)* NtOpenPartition;
	D_API(NtOpenPrivateNamespace)* NtOpenPrivateNamespace;
	D_API(NtOpenProcess)* NtOpenProcess;
	D_API(NtOpenProcessToken)* NtOpenProcessToken;
	D_API(NtOpenProcessTokenEx)* NtOpenProcessTokenEx;
	D_API(NtOpenRegistryTransaction)* NtOpenRegistryTransaction;
	D_API(NtOpenResourceManager)* NtOpenResourceManager;
	D_API(NtOpenSection)* NtOpenSection;
	D_API(NtOpenSemaphore)* NtOpenSemaphore;
	D_API(NtOpenSession)* NtOpenSession;
	D_API(NtOpenSymbolicLinkObject)* NtOpenSymbolicLinkObject;
	D_API(NtOpenThread)* NtOpenThread;
	D_API(NtOpenThreadToken)* NtOpenThreadToken;
	D_API(NtOpenThreadTokenEx)* NtOpenThreadTokenEx;
	D_API(NtOpenTimer)* NtOpenTimer;
	D_API(NtOpenTransaction)* NtOpenTransaction;
	D_API(NtOpenTransactionManager)* NtOpenTransactionManager;
	D_API(NtPlugPlayControl)* NtPlugPlayControl;
	D_API(NtPowerInformation)* NtPowerInformation;
	D_API(NtPrePrepareComplete)* NtPrePrepareComplete;
	D_API(NtPrePrepareEnlistment)* NtPrePrepareEnlistment;
	D_API(NtPrepareComplete)* NtPrepareComplete;
	D_API(NtPrepareEnlistment)* NtPrepareEnlistment;
	D_API(NtPrivilegeCheck)* NtPrivilegeCheck;
	D_API(NtPrivilegeObjectAuditAlarm)* NtPrivilegeObjectAuditAlarm;
	D_API(NtPrivilegedServiceAuditAlarm)* NtPrivilegedServiceAuditAlarm;
	D_API(NtPropagationComplete)* NtPropagationComplete;
	D_API(NtPropagationFailed)* NtPropagationFailed;
	D_API(NtProtectVirtualMemory)* NtProtectVirtualMemory;
	D_API(NtPssCaptureVaSpaceBulk)* NtPssCaptureVaSpaceBulk;
	D_API(NtPulseEvent)* NtPulseEvent;
	D_API(NtQueryAttributesFile)* NtQueryAttributesFile;
	D_API(NtQueryAuxiliaryCounterFrequency)* NtQueryAuxiliaryCounterFrequency;
	D_API(NtQueryBootEntryOrder)* NtQueryBootEntryOrder;
	D_API(NtQueryBootOptions)* NtQueryBootOptions;
	D_API(NtQueryDebugFilterState)* NtQueryDebugFilterState;
	D_API(NtQueryDefaultLocale)* NtQueryDefaultLocale;
	D_API(NtQueryDefaultUILanguage)* NtQueryDefaultUILanguage;
	D_API(NtQueryDirectoryFile)* NtQueryDirectoryFile;
	D_API(NtQueryDirectoryFileEx)* NtQueryDirectoryFileEx;
	D_API(NtQueryDirectoryObject)* NtQueryDirectoryObject;
	D_API(NtQueryDriverEntryOrder)* NtQueryDriverEntryOrder;
	D_API(NtQueryEaFile)* NtQueryEaFile;
	D_API(NtQueryEvent)* NtQueryEvent;
	D_API(NtQueryFullAttributesFile)* NtQueryFullAttributesFile;
	D_API(NtQueryInformationAtom)* NtQueryInformationAtom;
	D_API(NtQueryInformationByName)* NtQueryInformationByName;
	D_API(NtQueryInformationEnlistment)* NtQueryInformationEnlistment;
	D_API(NtQueryInformationFile)* NtQueryInformationFile;
	D_API(NtQueryInformationJobObject)* NtQueryInformationJobObject;
	D_API(NtQueryInformationPort)* NtQueryInformationPort;
	D_API(NtQueryInformationProcess)* NtQueryInformationProcess;
	D_API(NtQueryInformationResourceManager)* NtQueryInformationResourceManager;
	D_API(NtQueryInformationThread)* NtQueryInformationThread;
	D_API(NtQueryInformationToken)* NtQueryInformationToken;
	D_API(NtQueryInformationTransaction)* NtQueryInformationTransaction;
	D_API(NtQueryInformationTransactionManager)* NtQueryInformationTransactionManager;
	D_API(NtQueryInformationWorkerFactory)* NtQueryInformationWorkerFactory;
	D_API(NtQueryInstallUILanguage)* NtQueryInstallUILanguage;
	D_API(NtQueryIntervalProfile)* NtQueryIntervalProfile;
	D_API(NtQueryIoCompletion)* NtQueryIoCompletion;
	D_API(NtQueryKey)* NtQueryKey;
	D_API(NtQueryLicenseValue)* NtQueryLicenseValue;
	D_API(NtQueryMultipleValueKey)* NtQueryMultipleValueKey;
	D_API(NtQueryMutant)* NtQueryMutant;
	D_API(NtQueryObject)* NtQueryObject;
	D_API(NtQueryOpenSubKeys)* NtQueryOpenSubKeys;
	D_API(NtQueryOpenSubKeysEx)* NtQueryOpenSubKeysEx;
	D_API(NtQueryPerformanceCounter)* NtQueryPerformanceCounter;
	D_API(NtQueryPortInformationProcess)* NtQueryPortInformationProcess;
	D_API(NtQueryQuotaInformationFile)* NtQueryQuotaInformationFile;
	D_API(NtQuerySection)* NtQuerySection;
	D_API(NtQuerySecurityAttributesToken)* NtQuerySecurityAttributesToken;
	D_API(NtQuerySecurityObject)* NtQuerySecurityObject;
	D_API(NtQuerySecurityPolicy)* NtQuerySecurityPolicy;
	D_API(NtQuerySemaphore)* NtQuerySemaphore;
	D_API(NtQuerySymbolicLinkObject)* NtQuerySymbolicLinkObject;
	D_API(NtQuerySystemEnvironmentValue)* NtQuerySystemEnvironmentValue;
	D_API(NtQuerySystemEnvironmentValueEx)* NtQuerySystemEnvironmentValueEx;
	D_API(NtQuerySystemInformation)* NtQuerySystemInformation;
	D_API(NtQuerySystemInformationEx)* NtQuerySystemInformationEx;
	D_API(NtQueryTimer)* NtQueryTimer;
	D_API(NtQueryTimerResolution)* NtQueryTimerResolution;
	D_API(NtQueryValueKey)* NtQueryValueKey;
	D_API(NtQueryVirtualMemory)* NtQueryVirtualMemory;
	D_API(NtQueryVolumeInformationFile)* NtQueryVolumeInformationFile;
	D_API(NtQueryWnfStateData)* NtQueryWnfStateData;
	D_API(NtQueryWnfStateNameInformation)* NtQueryWnfStateNameInformation;
	D_API(NtQueueApcThread)* NtQueueApcThread;
	D_API(NtQueueApcThreadEx)* NtQueueApcThreadEx;
	D_API(NtQueueApcThreadEx2)* NtQueueApcThreadEx2;
	D_API(NtRaiseException)* NtRaiseException;
	D_API(NtRaiseHardError)* NtRaiseHardError;
	D_API(NtReadFile)* NtReadFile;
	D_API(NtReadFileScatter)* NtReadFileScatter;
	D_API(NtReadOnlyEnlistment)* NtReadOnlyEnlistment;
	D_API(NtReadRequestData)* NtReadRequestData;
	D_API(NtReadVirtualMemory)* NtReadVirtualMemory;
	D_API(NtRecoverEnlistment)* NtRecoverEnlistment;
	D_API(NtRecoverResourceManager)* NtRecoverResourceManager;
	D_API(NtRecoverTransactionManager)* NtRecoverTransactionManager;
	D_API(NtRegisterProtocolAddressInformation)* NtRegisterProtocolAddressInformation;
	D_API(NtRegisterThreadTerminatePort)* NtRegisterThreadTerminatePort;
	D_API(NtReleaseKeyedEvent)* NtReleaseKeyedEvent;
	D_API(NtReleaseMutant)* NtReleaseMutant;
	D_API(NtReleaseSemaphore)* NtReleaseSemaphore;
	D_API(NtReleaseWorkerFactoryWorker)* NtReleaseWorkerFactoryWorker;
	D_API(NtRemoveIoCompletion)* NtRemoveIoCompletion;
	D_API(NtRemoveIoCompletionEx)* NtRemoveIoCompletionEx;
	D_API(NtRemoveProcessDebug)* NtRemoveProcessDebug;
	D_API(NtRenameKey)* NtRenameKey;
	D_API(NtRenameTransactionManager)* NtRenameTransactionManager;
	D_API(NtReplaceKey)* NtReplaceKey;
	D_API(NtReplacePartitionUnit)* NtReplacePartitionUnit;
	D_API(NtReplyPort)* NtReplyPort;
	D_API(NtReplyWaitReceivePort)* NtReplyWaitReceivePort;
	D_API(NtReplyWaitReceivePortEx)* NtReplyWaitReceivePortEx;
	D_API(NtReplyWaitReplyPort)* NtReplyWaitReplyPort;
	D_API(NtRequestPort)* NtRequestPort;
	D_API(NtRequestWaitReplyPort)* NtRequestWaitReplyPort;
	D_API(NtResetEvent)* NtResetEvent;
	D_API(NtResetWriteWatch)* NtResetWriteWatch;
	D_API(NtRestoreKey)* NtRestoreKey;
	D_API(NtResumeProcess)* NtResumeProcess;
	D_API(NtResumeThread)* NtResumeThread;
	D_API(NtRevertContainerImpersonation)* NtRevertContainerImpersonation;
	D_API(NtRollbackComplete)* NtRollbackComplete;
	D_API(NtRollbackEnlistment)* NtRollbackEnlistment;
	D_API(NtRollbackRegistryTransaction)* NtRollbackRegistryTransaction;
	D_API(NtRollbackTransaction)* NtRollbackTransaction;
	D_API(NtRollforwardTransactionManager)* NtRollforwardTransactionManager;
	D_API(NtSaveKey)* NtSaveKey;
	D_API(NtSaveKeyEx)* NtSaveKeyEx;
	D_API(NtSaveMergedKeys)* NtSaveMergedKeys;
	D_API(NtSecureConnectPort)* NtSecureConnectPort;
	D_API(NtSerializeBoot)* NtSerializeBoot;
	D_API(NtSetBootEntryOrder)* NtSetBootEntryOrder;
	D_API(NtSetBootOptions)* NtSetBootOptions;
	D_API(NtSetCachedSigningLevel)* NtSetCachedSigningLevel;
	D_API(NtSetCachedSigningLevel2)* NtSetCachedSigningLevel2;
	D_API(NtSetContextThread)* NtSetContextThread;
	D_API(NtSetDebugFilterState)* NtSetDebugFilterState;
	D_API(NtSetDefaultHardErrorPort)* NtSetDefaultHardErrorPort;
	D_API(NtSetDefaultLocale)* NtSetDefaultLocale;
	D_API(NtSetDefaultUILanguage)* NtSetDefaultUILanguage;
	D_API(NtSetDriverEntryOrder)* NtSetDriverEntryOrder;
	D_API(NtSetEaFile)* NtSetEaFile;
	D_API(NtSetEvent)* NtSetEvent;
	D_API(NtSetEventBoostPriority)* NtSetEventBoostPriority;
	D_API(NtSetHighEventPair)* NtSetHighEventPair;
	D_API(NtSetHighWaitLowEventPair)* NtSetHighWaitLowEventPair;
	D_API(NtSetIRTimer)* NtSetIRTimer;
	D_API(NtSetInformationDebugObject)* NtSetInformationDebugObject;
	D_API(NtSetInformationEnlistment)* NtSetInformationEnlistment;
	D_API(NtSetInformationFile)* NtSetInformationFile;
	D_API(NtSetInformationJobObject)* NtSetInformationJobObject;
	D_API(NtSetInformationKey)* NtSetInformationKey;
	D_API(NtSetInformationObject)* NtSetInformationObject;
	D_API(NtSetInformationProcess)* NtSetInformationProcess;
	D_API(NtSetInformationResourceManager)* NtSetInformationResourceManager;
	D_API(NtSetInformationSymbolicLink)* NtSetInformationSymbolicLink;
	D_API(NtSetInformationThread)* NtSetInformationThread;
	D_API(NtSetInformationToken)* NtSetInformationToken;
	D_API(NtSetInformationTransaction)* NtSetInformationTransaction;
	D_API(NtSetInformationTransactionManager)* NtSetInformationTransactionManager;
	D_API(NtSetInformationVirtualMemory)* NtSetInformationVirtualMemory;
	D_API(NtSetInformationWorkerFactory)* NtSetInformationWorkerFactory;
	D_API(NtSetIntervalProfile)* NtSetIntervalProfile;
	D_API(NtSetIoCompletion)* NtSetIoCompletion;
	D_API(NtSetIoCompletionEx)* NtSetIoCompletionEx;
	D_API(NtSetLdtEntries)* NtSetLdtEntries;
	D_API(NtSetLowEventPair)* NtSetLowEventPair;
	D_API(NtSetLowWaitHighEventPair)* NtSetLowWaitHighEventPair;
	D_API(NtSetQuotaInformationFile)* NtSetQuotaInformationFile;
	D_API(NtSetSecurityObject)* NtSetSecurityObject;
	D_API(NtSetSystemEnvironmentValue)* NtSetSystemEnvironmentValue;
	D_API(NtSetSystemEnvironmentValueEx)* NtSetSystemEnvironmentValueEx;
	D_API(NtSetSystemInformation)* NtSetSystemInformation;
	D_API(NtSetSystemPowerState)* NtSetSystemPowerState;
	D_API(NtSetSystemTime)* NtSetSystemTime;
	D_API(NtSetThreadExecutionState)* NtSetThreadExecutionState;
	D_API(NtSetTimer)* NtSetTimer;
	D_API(NtSetTimer2)* NtSetTimer2;
	D_API(NtSetTimerEx)* NtSetTimerEx;
	D_API(NtSetTimerResolution)* NtSetTimerResolution;
	D_API(NtSetUuidSeed)* NtSetUuidSeed;
	D_API(NtSetValueKey)* NtSetValueKey;
	D_API(NtSetVolumeInformationFile)* NtSetVolumeInformationFile;
	D_API(NtSetWnfProcessNotificationEvent)* NtSetWnfProcessNotificationEvent;
	D_API(NtShutdownSystem)* NtShutdownSystem;
	D_API(NtShutdownWorkerFactory)* NtShutdownWorkerFactory;
	D_API(NtSignalAndWaitForSingleObject)* NtSignalAndWaitForSingleObject;
	D_API(NtSinglePhaseReject)* NtSinglePhaseReject;
	D_API(NtStartProfile)* NtStartProfile;
	D_API(NtStopProfile)* NtStopProfile;
	D_API(NtSubscribeWnfStateChange)* NtSubscribeWnfStateChange;
	D_API(NtSuspendProcess)* NtSuspendProcess;
	D_API(NtSuspendThread)* NtSuspendThread;
	D_API(NtSystemDebugControl)* NtSystemDebugControl;
	D_API(NtTerminateEnclave)* NtTerminateEnclave;
	D_API(NtTerminateJobObject)* NtTerminateJobObject;
	D_API(NtTerminateProcess)* NtTerminateProcess;
	D_API(NtTerminateThread)* NtTerminateThread;
	D_API(NtTestAlert)* NtTestAlert;
	D_API(NtThawRegistry)* NtThawRegistry;
	D_API(NtThawTransactions)* NtThawTransactions;
	D_API(NtTraceControl)* NtTraceControl;
	D_API(NtTraceEvent)* NtTraceEvent;
	D_API(NtTranslateFilePath)* NtTranslateFilePath;
	D_API(NtUmsThreadYield)* NtUmsThreadYield;
	D_API(NtUnloadDriver)* NtUnloadDriver;
	D_API(NtUnloadKey)* NtUnloadKey;
	D_API(NtUnloadKey2)* NtUnloadKey2;
	D_API(NtUnloadKeyEx)* NtUnloadKeyEx;
	D_API(NtUnlockFile)* NtUnlockFile;
	D_API(NtUnlockVirtualMemory)* NtUnlockVirtualMemory;
	D_API(NtUnmapViewOfSection)* NtUnmapViewOfSection;
	D_API(NtUnmapViewOfSectionEx)* NtUnmapViewOfSectionEx;
	D_API(NtUnsubscribeWnfStateChange)* NtUnsubscribeWnfStateChange;
	D_API(NtUpdateWnfStateData)* NtUpdateWnfStateData;
	D_API(NtVdmControl)* NtVdmControl;
	D_API(NtWaitForAlertByThreadId)* NtWaitForAlertByThreadId;
	D_API(NtWaitForDebugEvent)* NtWaitForDebugEvent;
	D_API(NtWaitForKeyedEvent)* NtWaitForKeyedEvent;
	D_API(NtWaitForMultipleObjects)* NtWaitForMultipleObjects;
	D_API(NtWaitForMultipleObjects32)* NtWaitForMultipleObjects32;
	D_API(NtWaitForSingleObject)* NtWaitForSingleObject;
	D_API(NtWaitForWorkViaWorkerFactory)* NtWaitForWorkViaWorkerFactory;
	D_API(NtWaitHighEventPair)* NtWaitHighEventPair;
	D_API(NtWaitLowEventPair)* NtWaitLowEventPair;
	D_API(NtWorkerFactoryWorkerReady)* NtWorkerFactoryWorkerReady;
	D_API(NtWriteFile)* NtWriteFile;
	D_API(NtWriteFileGather)* NtWriteFileGather;
	D_API(NtWriteRequestData)* NtWriteRequestData;
	D_API(NtWriteVirtualMemory)* NtWriteVirtualMemory;
	D_API(NtYieldExecution)* NtYieldExecution;
} NT_API, *PNT_API;
...
#define SYSCALL_TABLE_ENTRY(table, index) \
    (SYSCALL_ENTRY*)((char*)&(table)->Entries + (index) * sizeof(SYSCALL_ENTRY))
